...
out_dir = '/home/bayrakrg/neurdy/pycharm/multi-task-physio/IPMI2021/k_fold_task_files/'

train_files = []
test_files = []

# grabbing data.keys() from combine_list.py
id_list = list(data.keys())
random.shuffle(id_list)
all_id_list = deepcopy(id_list)
# k-fold
k = 5
parc = round(len(id_list) / k)

for i in range(0, k):
    test_files = []
    for l, id in enumerate(id_list):
        if len(test_files) < parc:
            test_files.extend([x for x in id_list if id in x])

    train_files = [item for item in all_id_list if item not in test_files]
    files = [item for item in id_list if item not in test_files]
