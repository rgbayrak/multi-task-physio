...
'''
This script is for a data loader that randomly crops the input to be of different sizes.
'''

def get_roi_len(dirs):
    roi_path = "/bigdata/HCP_task/social_min+prepro/bpf-ds/"
    roi_len = 0
    for dir in dirs:
...
def get_sub(path):
    ...
    for subline in sublines:
        ...
        hr_fold.append(subline.replace('rois', 'HR_filtds'))
        rv_fold.append(subline.replace('rois', 'RV_filtds'))
...
def get_dictionary(fold):
    roi_path = os.path.join("/bigdata/HCP_task/social_min+prepro/bpf-ds/")
    hr_path = "/bigdata/HCP_task/social_min+prepro/bpf-ds/physio/HR_filt_ds/"
    rv_path = "/bigdata/HCP_task/social_min+prepro/bpf-ds/physio/RV_filt_ds/"
    fold_path = os.path.join("/home/bayrakrg/neurdy/pycharm/multi-task-physio/ISBI2020/social_files/", fold)
    ...
    for i, d in enumerate(gm_fold):
        ...
        clust_list = sorted(os.listdir(roi_path))
    ...
    for subj in data:
        ...
        scan_order = []
        for path in paths:
            scan_order.append(path.lstrip('/bigdata/HCP_task/social_min+prepro/bpf-ds/findlab90/rois_').rstrip('.mat'))
...
class data_to_tensor():
    ...
    def __getitem__(self, idx):
        ...
        # gm = single[self.roi_list[0] + '_mat'][self.idx_list[idx][1]]['roi_dat'][20:580, :]
        # ngm = single[self.roi_list[1] + '_mat'][self.idx_list[idx][1]]['roi_dat_wmcsf'][20:580, :]
        # hr = single['HR_filt_ds'][self.idx_list[idx][1]]['hr_filt_ds'][20:580, :]  # trimmed
        # rv = single['RV_filt_ds'][self.idx_list[idx][1]]['rv_filt_ds'][20:580, :]  # trimmed

        #
        gm = single[self.roi_list[0]][self.idx_list[idx][1]]['roi_dat']
        ngm = single[self.roi_list[1]][self.idx_list[idx][1]]['roi_dat']
        hr = single['HR_filt_ds'][self.idx_list[idx][1]]['hr_filt_ds'] # trimmed
        rv = single['RV_filt_ds'][self.idx_list[idx][1]]['rv_filt_ds']  # trimmed
        #
        # gm = np.vstack((gm, np.zeros((423,90))))
        # ngm = np.vstack((ngm, np.zeros((423,12))))
        # hr = np.vstack((hr, np.zeros((423,1))))
        # rv = np.vstack((rv, np.zeros((423,1))))
