...
class UNet(nn.Module):
    def __init__(self, in_channel, out_channel, opt):
        ...
        # first split
        self.dc51 = self.decoder(128 + 256, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc41 = self.decoder(133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc31 = self.decoder(133, 133, kernel_size=2, stride=2, bias=True)
        self.dc21 = self.decoder(64 + 133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc11 = self.decoder(133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc01 = self.decoder(133, 1, kernel_size=1, stride=1, bias=True)
        # second split
        self.dc52 = self.decoder(128 + 256, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc42 = self.decoder(133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc32 = self.decoder(133, 133, kernel_size=2, stride=2, bias=True)
        self.dc22 = self.decoder(64 + 133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc12 = self.decoder(133, 133, kernel_size=3, stride=1, padding=1, bias=True)
        self.dc02 = self.decoder(133, 1, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        d31 = torch.cat((self.dc31(d41), syn0), 1)
        # del d4, syn0
        ...
        d32 = torch.cat((self.dc32(d42), syn0), 1)
        del d42, syn0
        # print("block d3 size = %s" % (str(d3.size())))
