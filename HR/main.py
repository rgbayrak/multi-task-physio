...
def train_model(opt):
    ...
    with tqdm(total=opt.epoch) as pbar:
        for epoch in range(1, opt.epoch + 1):
            ...
            avg_loss_hr, target_hr, pred_hr = train(model, device, train_loader, optim, opt)

            avg_val_loss, target_hrs, pred_hrs = test(model, device, val_loader, opt)
            ...
            # progress bar
            pbar.set_description(
                "Epoch {}  \t Avg. Training >> \t Loss HR: {:.4f} \t Avg. Val. Loss: {:.4f}".format(epoch, avg_loss_hr, avg_val_loss))
            pbar.update(1)
...
def test_model(opt):
    ...
    avg_loss, target_hrs, preds = test(model, device, test_loader, opt)
    ...
    # norm_pred = (hr - hr.mean(axis=0)) / hr.std(axis=0)  # z-score normalization
    #
    # plt.plot(np.arange(0, 560), norm_pred)
    # plt.plot(np.arange(0, 560), norm_target)
    # plt.legend(['Prediction', 'Target'])
    # corr_coeff = sp.stats.pearsonr(preds[n][:].squeeze(), target_hrs[n].squeeze())
    # plt.title(
    #     "{}th_{} | corr_coeff={} ".format(n, opt.test_fold.rstrip('.txt'), corr_coeff[0]))
    # plt.show()

    # Save statistics
    test_corr_file = '{}results/{}/hr_corr_splits_{}'.format(opt.out_dir, opt.uni_id, opt.test_fold)
...
def main():
    ...
    parser.add_argument('--multi', type=str, default='only_hr')
    parser.add_argument('--uni_id', type=str, default='Bi-LSTM-HR-only')
    ...
    parser.add_argument('--out_dir', type=str, default='/home/bayrakrg/neurdy/pycharm/HR/out/', help='Path to output directory')
